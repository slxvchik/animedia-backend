...
import dev.animedia.contentservice.infrastructure.persistence.genre.repository.JpaGenreRepository;
...
import dev.animedia.contentservice.infrastructure.persistence.status.repository.JpaStatusRepository;
...
public class ContentQueryRepositoryImpl implements ContentQueryRepository {
	...
	private Status fetchStatus(UUID statusId, @Nullable String languageCode) {
		StatusEntity se = jpaStatusRepository.findById(statusId, null, languageCode);
	...
	private Set<Status> fetchStatuses(List<UUID> statusIdList, @Nullable String languageCode) {
		List<StatusEntity> seList = jpaStatusRepository.findByIdList(statusIdList, null, languageCode);
	...
	private Set<Genre> fetchGenres(List<UUID> genreIdList, @Nullable String languageCode) {
		List<GenreEntity> geList = jpaGenreRepository.findByIdList(genreIdList, languageCode, null);
